...
t_NtQueryObject GetNtQueryObject(void)
{
...
DWORD GetNtPathFromHandle(HANDLE h_File, wchar_t* widepath)
{
    ...
    {
        return swprintf(widepath, WIDE_PATH_MAX, L"\\Device\\Console%04X", WIDE_PATH_MAX, (DWORD)(DWORD_PTR)h_File);
    }

    BYTE  u8_Buffer[2000];
    DWORD u32_ReqLength = 0;

	OBJECT_NAME_INFORMATION* pObjNameInfo = (OBJECT_NAME_INFORMATION*)u8_Buffer;
    UNICODE_STRING* pk_Info = &pObjNameInfo->Name;
    pk_Info->Buffer = 0;
    pk_Info->Length = 0;
    ...
    GetNtQueryObject()(h_File, ObjectNameInformation, u8_Buffer, sizeof(u8_Buffer), &u32_ReqLength);
    ...
    return swprintf(widepath, WIDE_PATH_MAX, L"\0", WIDE_PATH_MAX);
}
...
DWORD GetDosPathFromNtPath(const WCHAR* u16_NTPath, wchar_t* widepath)
{
    DWORD u32_Error;
	DWORD len = 0;
    ...
    {
        HKEY h_Key; 
		u32_Error = RegOpenKeyEx(HKEY_LOCAL_MACHINE, L"Hardware\\DeviceMap\\SerialComm", 0, KEY_QUERY_VALUE, &h_Key);
        if (0 != u32_Error)
            return u32_Error;
		...
		u32_Error = RegQueryValueEx(h_Key, u16_NTPath, 0, &u32_Type, (BYTE*)u16_ComPort, &u32_Size);
        if (0 != u32_Error)
        {
            RegCloseKey(h_Key);
		...
		len = swprintf(widepath, WIDE_PATH_MAX, L"%s", WIDE_PATH_MAX, u16_ComPort);
        RegCloseKey(h_Key);
        return len;
    }
    ...
    while (u16_Drv[0])
    {
        ...
        {
			len = swprintf(widepath, WIDE_PATH_MAX, L"%s%d", WIDE_PATH_MAX, u16_Drv, s32_Len);
            return len;
        }
...
#else
	len = GetNtPathFromHandle(FileHandle, widepath);
#endif	
	if(len == 0) {
